...
public class testSA {
	...
	public void testNoMigration() {
		...
		SimulationAnneal sa= new SimulationAnneal(pCPU, pVM, vAssignOld, oldPMInUse, newPMInUse,
				targetUtilization) ;
		sa.anneal();
	...
	public void test2Migrations() {
		...
		SimulationAnneal sa= new SimulationAnneal(pCPU, pVM, vAssignOldHere, oldPMInUse, newPMInUse,
				targetUtilization) ;
		sa.anneal();
		int[] result = sa.getAssignment();
		assertTrue(result.length==vAssignOld.length);
