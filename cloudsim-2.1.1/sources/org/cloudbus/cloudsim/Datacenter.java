...
public class Datacenter extends SimEntity {
	...
	protected void processDataAdd(SimEvent ev, boolean ack) {
       if (ev == null) {
        ...
        PhysicalFile file = (PhysicalFile) pack[0]; // get the file
    ...
    protected double predictFileTransferTime(List<String> requiredFiles) {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < getStorageList().size(); i++) {
				...
				PhysicalFile tempFile = tempStorage.getFile(fileName);
				if (tempFile != null) {
    ...
    */
    public int addFile(PhysicalFile file) {
        if (file == null) {
     ...
     */
    protected boolean contains(PhysicalFile file) {
        if (file == null) {
            return false;
        }
