...
public class PhysicalFile {
     ...
     *      </ul>
     */
    public PhysicalFile(String fileName, int fileSize) throws ParameterException {
     ...
     *                            <tt>null</tt>
     */
    public PhysicalFile(PhysicalFile file) throws ParameterException {
        if (file == null) {
     ...
     */
    public PhysicalFile makeReplica() {
        return makeCopy();
    }
     ...
     *          or <tt>null</tt> if an error occurs
     */
    public PhysicalFile makeMasterCopy() {
        PhysicalFile file = makeCopy();
        if (file != null) {
     ...
     *          or <tt>null</tt> if an error occurs
     */
    private PhysicalFile makeCopy() {
        PhysicalFile file = null;
        try {
            file = new PhysicalFile(name, attribute.getFileSize());
            FileAttribute fileAttr = file.getFileAttribute();
            attribute.copyValue(fileAttr);
            fileAttr.setMasterCopy(false);   // set this file to replica
        } catch (Exception e) {
     ...
     * @return <tt>true</tt> if successful, <tt>false</tt> otherwise
     * @see gridsim.datagrid.storage.Storage#addFile(PhysicalFile)
     * @see gridsim.datagrid.storage.Storage#addFile(List)
     * @see gridsim.datagrid.storage.Storage#addReservedFile(PhysicalFile)
     * @see gridsim.datagrid.storage.Storage#deleteFile(PhysicalFile)
     * @see gridsim.datagrid.storage.Storage#deleteFile(String)
     * @see gridsim.datagrid.storage.Storage#deleteFile(String, PhysicalFile)
     * @see gridsim.datagrid.storage.Storage#getFile(String)
     * @see gridsim.datagrid.storage.Storage#renameFile(PhysicalFile, String)
     */
